...
{
    internal static class AnimationBaker
    {
        [Serializable]
        public struct BakedData
        {
            ...
            public Texture2D GetPositionMap => positionMaps[0];
        }
        ...
        public static BakedData Bake(GameObject model, AnimationClip[] animations, bool applyRootMotion, int fps, int textureWidth)
        {
            ...
            {
                int frames = Mathf.FloorToInt(fps * animations[i].length);
            ...
            for (int i = 0; i < animations.Length; i++)
            {
                ...
                animationInfo.frames = Mathf.FloorToInt(fps * clip.length);

                BakedData bd = Bake(model, clip, animationInfo);
        ...
        private static BakedData Bake(this GameObject model, AnimationClip animationClip, AnimationInfo animationInfo)
        {
            Mesh mesh = new Mesh
            {
            ...
            if (model.TryGetComponent(out UnityEngine.Animator animator))
            {
        ...
        private static BakedAnimation BakeAnimation(this GameObject model, AnimationClip animationClip, AnimationInfo animationInfo)
        {
