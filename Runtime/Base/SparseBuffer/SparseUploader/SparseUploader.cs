using Scellecs.Morpeh.Graphics.UnityInternal;
...
namespace Scellecs.Morpeh.Graphics
{
    ...
    /// </remarks>
    [StructLayout(LayoutKind.Sequential)]
    internal unsafe struct ThreadedSparseUploader
    {
        // TODO: safety handle?
        ...
        public void AddUpload(ref UploadDataSource uploadArgs, int offsetInBytes, int repeatCount = 1)
        {
            ...
            {
                var entityId = filter[filterOffset + i];
                var srcOffset = srcData.stashMap.TryGetIndex(entityId.Id) * sizeOf;
                var srcPtr = srcData.data + srcOffset;
                var dstPtr = srcBuffer + i * sizeOf;
                UnsafeUtility.MemCpy(dstPtr, srcPtr, sizeOf);
            }
        ...
        public void AddMatrixUpload(ref UploadDataSource uploadArgs, int offsetInBytes, int offsetInverse)
        {
            var count = uploadArgs.count;
            var opsize = UnsafeUtility.SizeOf<Operation>();
            var size = SIZE_OF_MATRIX3X4 * count;
            var allocSucceeded = TryAlloc(opsize, size, out var dst, out var operationOffset, out var dataOffset);
            ...
            {
                var entityId = filter[filterOffset + i];
                var data = srcData.Get(entityId);
                srcBuffer[i] = new float3x4(data.c0.xyz, data.c1.xyz, data.c2.xyz, data.c3.xyz);
            }

            var uploadType = (offsetInverse == -1) ? (uint)OperationType.Matrix_3x4 : (uint)OperationType.Matrix_Inverse_3x4;
    ...
    /// </summary>
    internal struct SparseUploaderStats
    {
        /// <summary>
    ...
    /// </remarks>
    internal unsafe struct SparseUploader : IDisposable
    {
        ...
        public SparseUploader(GraphicsBuffer destinationBuffer, int bufferChunkSize = 16 * 1024 * 1024)
        {
            ...
            m_ThreadData = (ThreadedSparseUploaderData*)UnityInternalUnsafeUtility.Allocate(sizeof(ThreadedSparseUploaderData),
        ...
        public void Dispose()
        {
            ...
            UnityInternalUnsafeUtility.Free(m_ThreadData, Allocator.Persistent);
        }
